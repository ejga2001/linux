...
ssize_t __import_iovec_unchecked(int type, const struct iovec __user *uvec,
		 unsigned nr_segs, unsigned fast_segs, struct iovec **iovp,
		 struct iov_iter *i)
{
	ssize_t total_len = 0;
	...
	iov = iovec_from_user(uvec, nr_segs, fast_segs, *iovp, false);
	if (IS_ERR(iov)) {
		*iovp = NULL;
		return PTR_ERR(iov);
	}
...
EXPORT_SYMBOL(__import_iovec_unchecked);

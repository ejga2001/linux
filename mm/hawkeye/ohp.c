	...
	for(index = MAX_BINS - 1; index > 0; index--) {
		list_for_each_entry(mm, &ohp_scan.mm_head, ohp_list) {
			...
			*src = mm;
			...
			list_del(&mm->ohp_list);
			list_add_tail(&mm->ohp_list, &ohp_scan.mm_head);
			pr_debug("task: %s, mm@0x%lx, addr@0x%lx, bin: %d",
					(mm && mm->owner) ? mm->owner->comm : "N/A",
					(unsigned long)mm,
					(unsigned long)kaddr->address, index);
			goto found;
		}
	}
	pr_debug("empty bins!");
				...
				unsigned long idle_msecs)
{
	...
	//if (sleep_msecs < 5000)
	//	sleep_msecs = 5000;
	//if (sleep_msecs < 30000)
	//	sleep_msecs = 30000;
	sleep_msecs = 1000;

	pr_debug("busy: %ld ms sleep: %ld ms\n", busy_msecs, sleep_msecs);
...
static void kbinmanager_do_scan(unsigned long *idle_msecs)
{
	...
	while (true) {
		...
		kbinmanager_wait_scan_period(30000);
		//*idle_msecs += 30000;
	...
	for (index = MAX_BINS - 1; index >=0; index--) {
		list_for_each_entry_safe(kaddr, tmp,
			...
			if (nr_accessed < 0) {
				 ...
				 */
				pr_debug("removing kaddr 0x%lx", kaddr->address);
				list_del(&kaddr->entry);
				mm->ohp.ohp_remaining -= 1;
				mm->ohp.count[index] -= 1;
				mm->ohp.invalid += 1;
			...
			//pr_debug("updating 0x%lx", kaddr->address);
